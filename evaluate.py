...
from src.models.cnn import MNIST_MLR
...
def get_random_test_samples(data_module: MNISTDataModule, num_samples: int = 9, seed: int = 42) -> Tuple[torch.Tensor, torch.Tensor]:
...
def create_prediction_plot(images: torch.Tensor, true_labels: torch.Tensor, predictions: torch.Tensor, save_path: str):
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate model predictions on test data")
    parser.add_argument("--experiment_path", type=str, required=True,
                       help="Path to experiment directory (e.g., experiments_mlr/2025-06-25_15-30)")
    parser.add_argument("--model_type", type=str, required=True, choices=["mnist_mlr", "simple_cnn"],
    ...
    try:
        ...
        # Setup data module
        data_module = MNISTDataModule(batch_size=32)  # batch_size doesn't matter for testing
        
        # Get random test samples
        print(f"Getting {args.num_samples} random test samples (seed={args.seed})...")
        images, true_labels = get_random_test_samples(data_module, args.num_samples, args.seed)
        ...
        print(f"Sample accuracy: {accuracy:.2%} ({(true_labels == predictions).sum().item()}/{len(true_labels)})")
        
        # Create save path
        experiment_name = Path(args.experiment_path).name
        save_path = Path(args.results_dir) / args.model_type / experiment_name / "prediction_visualization.png"
